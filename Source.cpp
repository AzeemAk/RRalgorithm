...
//Driver Code
int main() {
    int n, tq = 10, timer = 0, maxProccessIndex = 0;
    float avgWait = 0, avgTT = 0;
    int *processNumber, *arrival, *burst, *wait, *turn, *queue, *temp_burst;
    bool * complete;
    ...
    if (myFile.is_open()) {
        ...
        turn = new int[n];
        queue = new int[n];
        temp_burst = new int[n];                   //initializing arrays based on number of processes
        complete = new bool[n];
        for (int i = 0; i < n; i++)
        {
            myFile >> processNumber[i];
            myFile >> arrival[i];                      //reading in data from text file
            myFile >> burst[i];
